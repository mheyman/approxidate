...
static int gettimeofday(struct timeval * tp, struct timezone * tzp)
{
    ...
    if (tp)
    {
        ...
        uint64_t time = file_time.dwLowDateTime;
        time += static_cast<uint64_t>(file_time.dwHighDateTime) << 32;
        time -= EPOCH;
        time /= 10; // microseconds;
        tp->tv_sec = static_cast<long>(time / 1000000L);
        tp->tv_usec = static_cast<long>(time % 1000000L);
    }
...
static int try_number_or_month(char *s, char **end, long *val, int *definitely_month)
{
    if (isdigit(*s)) {
        *val = strtol(s, end, 10);
        *definitely_month = 0;
    ...
    for (int i = 0;; ++i) {
        if (i == sizeof(month_names) / sizeof(month_names[0])) {
            return 0;
        }

        int match = match_string(s, month_names[i]);
        if (match >= 3) {
            *val = i + 1;
            *end = s + match;
            *definitely_month = !0;
...
static int match_multi_number(unsigned long num, char c, const char *date, char *end, struct atm *tm)
{
    time_t now;
    long num2, num3, num4;
    int num2_definitely_month;
    int num3_definitely_month;
    num3 = -1;
    num4 = 0;

    if (try_number_or_month(end + 1, &end, &num2, &num2_definitely_month)) {
        if (*end == c ) {
            if(try_number_or_month(end + 1, &end, &num3, &num3_definitely_month)) {
                if (*end == '.') {
                    char *start = end + 1;
    ...
    case '.':
        ...
        if (num > 70) {
            /* yyyy-mm-dd? */
            if (!num3_definitely_month && is_date(num, num2, num3, NULL, now, tm))
                break;
            /* yyyy-dd-mm? */
            if (!num2_definitely_month && is_date(num, num3, num2, NULL, now, tm))
                break;
        }
